...
class CategorieController extends AbstractController
{
    ...
    public function new(Request $request, CategorieRepository $categorieRepository,SluggerInterface $slugger): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($icone) {
                $originalFilename = pathinfo($icone->getClientOriginalName(), PATHINFO_FILENAME);
                // this is needed to safely include the file name as part of the URL
                $safeFilename = $slugger->slug($originalFilename);
                $newFilename = $safeFilename.'-'.uniqid().'.'.$icone->guessExtension();

                // Move the file to the directory where brochures are stored
                try {
                    $icone->move(
                        $this->getParameter('category_icone_directory'),
                        $newFilename
                    );
